...
Long story short, the input is a <u>**5-dimensional image that is generated by the BEV representation of the LiDAR point cloud and the top-view image**</u>. The point cloud data from LiDAR is first projected onto the grid in the $x$-$y$ plane, and to each pixel the intensity and the gradient of the height value are assigned. This is then concatenated with the corresponding RGB image, resulting in a 5-dimensional input image. Mathematically speaking, each input tensor is
