...
Another possible solution is, simply reducing the dimensionality to 2D, and applying the conventional 2D convolution operation. This, as pointed out above, requires another step in order to minimize the information loss that is caused by dimension reduction. For example, the authors of [**PointPillars: Fast Encoders for Object Detection from Point Clouds**](https://arxiv.org/pdf/1812.05784.pdf) suggests a way of converting the point cloud to a pseudo-image, each of whose pixels is a 9-dimensional vector that includes the $z$ coordinate. including the height information. More on the PointPillars can be found from the following [post](https://youngwoong-cho.github.io).